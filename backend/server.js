...
app.post("/save-metadata", (req, res) => {
    const { id, file_name, title, file_url, word_counts, keywords, uploaded_at } = req.body
    ...
    const query = `INSERT INTO metadata (id, file_name, title, file_url, word_counts, keywords, uploaded_at) VALUES (?,?,?,?,?,?,?)`

    db.query(
        query,
        [id, file_name, title, fileUrlString, word_counts, keywords, uploaded_at],
        (err, result) => {
            if (err) {
